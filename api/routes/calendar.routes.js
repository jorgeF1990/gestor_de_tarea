...
router.post('/ticket/:id/sync-google', auth, async (req, res) => {
  try {
    ...
    if (!ticket.fecha_vencimiento) return res.status(400).json({ error: 'El ticket no tiene fecha de vencimiento' });
    ...
    const eventData = {
      ...
      startDateTime, endDateTime, timeZone: 'America/Buenos_Aires', location: 'Portfolio Investment',
    ...
    await ticket.save();
    res.json({ success: true, eventId: result.eventId, eventLink: result.eventLink, hangoutLink: result.hangoutLink });
  } catch (error) {
...
router.get('/auth/outlook', auth, async (req, res) => {
  try {
    ...
    console.error('Error iniciando auth Outlook:', error);
...
router.get('/auth/outlook/callback', async (req, res) => {
  try {
    ...
    console.error('Error en callback Outlook:', error);
...
router.post('/ticket/:id/sync-outlook', auth, async (req, res) => {
  try {
    ...
    ticket.last_calendar_sync = new Date();
    await ticket.save();
    res.json({ success: true, eventId: result.eventId, eventLink: result.eventLink, onlineMeetingUrl: result.onlineMeetingUrl });
  } catch (error) {
    console.error('Error creando evento en Outlook:', error);
    res.status(500).json({ error: error.message });
  }
});
